...
def get_arguments():
    ...
    parser.add_argument("--tfrecords_train_path", type=str, default="./sr16_aB_train.tfrecords", help="")
    parser.add_argument("--tfrecords_dev_path", type=str, default="./sr16_aB_dev.tfrecords", help="")
    parser.add_argument("--pkl_train_path", type=str, default="./sr16_aB_train.pkl", help="where to store the norm coefs.")
    parser.add_argument("--pkl_dev_path", type=str, default="./sr16_aB_dev.pkl", help="where to store the norm coefs.")
    ...
    parser.add_argument("--random_num", type=int, default=3000, help="")
...
def main():
    ...
    with tf.python_io.TFRecordWriter(args.tfrecords_train_path) as writer:
        for path in tqdm.tqdm(path_lst_train):
            try:
                example_str, value_tag = read_to_bytes(path=path, stats=norm_dict, sr=args.sr,
                                            frame_shift=args.frame_shift, frame_size=args.frame_size,
                                            n_fft=args.n_fft, window=args.window,
                                            mel_filterbank=mel_filterbank, floor_gate=args.floor_gate)
                if value_tag == True:
