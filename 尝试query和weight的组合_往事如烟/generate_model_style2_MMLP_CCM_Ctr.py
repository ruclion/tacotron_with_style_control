        ...
        with tf.variable_scope(name):
            ...
            def body(this_time, old_output_ta, old_alpha_ta, old_alpha_style_ta, old_weight_ta, old_weight_per_ta,
                ...
                with tf.variable_scope("weighting"):
                    weight_input = tf.concat([context, context_style, dec_pre_ed_inp], axis=-1)
                    weighting = tf.layers.dense(weight_input, 256, tf.nn.sigmoid)
                    weighting = tf.layers.dense(weighting, 128, tf.nn.sigmoid)
                    weighting = tf.layers.dense(weighting, 32, tf.nn.sigmoid)
                ...
                with tf.variable_scope('decoder_rnn'):
                    ...
                    weight_per = tf.Print(weight_per, [weight_per], message='weight_per')
