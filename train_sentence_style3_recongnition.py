...
def get_next_batch(sess, next_item):
    ...
    return t['txt'], t['txt_len'], t['frames'], t['log_mel'], t['log_stftm'], t['key']
...
def main():
    ...
    train_model = Tacotron(inp=inp, inp_mask=inp_mask, frame_mask = frame_mask, seq2seq_gtruth=seq2seq_gtruth, post_gtruth=post_gtruth,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        try:
            for cnt in tqdm.tqdm(range(already_step_eval + 1, hp.max_global_steps + 10)):
                # print('now is', cnt)
                pre_time = time.time()
                batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth, batch_key = get_next_batch(sess, train_next_item)
                # print('bug', batch_inp[0], 'len', batch_inp_mask[0], 'actual', batch_inp[0].shape)
                batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth = post_next_batch(batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth, train_meta)
