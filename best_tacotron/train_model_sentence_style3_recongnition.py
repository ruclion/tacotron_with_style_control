...
class Tacotron(Model):
    def __init__(self, inp, inp_mask, frame_mask, seq2seq_gtruth, post_gtruth, hyper_params=None, training=True, name='Tacotron', reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            # with tf.variable_scope("inp_att_embedding"):
            #     inp_att = EmbeddingLayer(train_sentence_max_num, self.hyper_params.styles_kind)(inp_id)
            #     inp_att = tf.nn.softmax(inp_att)
            ...
            with tf.variable_scope('recongnition_att'):
                ...
                outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=GRUCell(80), cell_bw=GRUCell(80),
                                                             inputs=seq2seq_gtruth, sequence_length=frame_mask,
                                                             dtype=seq2seq_gtruth.dtype,
                                                             parallel_iterations=unkonwn_parallel_iterations)
                outputs = tf.concat(outputs, axis=-1)
                outputs = tf.transpose(outputs, [1, 0, 2])
                static_recongnition_output = outputs[-1]
            ### Encoder [end]

            sentence_style_att = tf.layers.dense(static_recongnition_output, 80, tf.nn.tanh)
            sentence_style_att = tf.layers.dense(sentence_style_att, 40, tf.nn.tanh)
            sentence_style_att = tf.layers.dense(sentence_style_att, 10, tf.nn.softmax)
            self.att = sentence_style_att
            # sentence_style = tf.Print(sentence_style, [sentence_style[0]], message='sentence', summarize=10)
            sentence_style = tf.reduce_sum(tf.expand_dims(sentence_style_att, axis=-1) * self.style_token, axis=1)
            ...
            def body(this_time, old_context, old_output_ta, old_alpha_ta, old_weight_per_ta, old_state_tup):
                ...
                with tf.variable_scope('decoder_rnn'):
                    ...
                    weighting_context = context + sentence_style * 3
                    weight_per = tf.reduce_mean(tf.abs(sentence_style[0]) / (tf.abs(context[0]) + tf.abs(sentence_style[0])))
                ...
                return tf.add(this_time, 1), context, new_output_ta, new_alpha_ta,  new_weight_per_ta, new_state_tup

            # run loop
            _, _, seq2seq_output_ta, alpha_ta, weight_per_ta, *_ = tf.while_loop(cond, body, [init_time, init_context, init_output_ta,
                                                                               init_alpha_ta, init_weight_per_ta, init_state_tup],
                                                                  parallel_iterations=unkonwn_parallel_iterations)
    ...
    def get_scalar_summary(self, suffix):
        with tf.variable_scope('summary'):
            sums = [tf.summary.scalar('{}/loss'.format(suffix), self.loss),
                    ...
                    tf.summary.histogram('{}/alpha_style_0'.format(suffix), self.alpha_style_att[0]),
                    tf.summary.histogram('{}/alpha_style_2'.format(suffix), self.alpha_style_att[1]),
    ...
    def get_alpha_summary(self, suffix, num_img=2):
        with tf.variable_scope('summary'):
            sums = [tf.summary.image('{}/alpha'.format(suffix), self.alpha_output[:num_img]),
                    tf.summary.image('{}/alpha_style'.format(suffix), self.alpha_style_att[:num_img])]
            return tf.summary.merge(sums)
