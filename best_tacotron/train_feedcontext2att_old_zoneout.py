...
class Tacotron(Model):
    def __init__(self, inp, inp_mask, seq2seq_gtruth, post_gtruth, hyper_params=None, training=True, name='Tacotron', reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            ### Decoder [begin]
            att_cell = ZoneoutWrapper(sGRUCell(256), 0.1, training)
    ...
    def get_alpha_summary(self, suffix, num_img=1):
        with tf.variable_scope('summary'):
            return tf.summary.image('{}/alpha'.format(suffix), self.alpha_output[:num_img])
