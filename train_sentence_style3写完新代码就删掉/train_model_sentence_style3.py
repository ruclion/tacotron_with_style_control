...
def l1_loss(gtruth, predict):
    ...
    return tf.reduce_mean(tf.reduce_sum(tf.abs(gtruth - predict), axis=1))
...
class Tacotron(Model):
    def __init__(self, inp, inp_mask, seq2seq_gtruth, post_gtruth, hyper_params=None, training=True, name='Tacotron', reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            def body(this_time, old_context, old_output_ta, old_alpha_ta, old_weight_ta, old_weight_per_ta, old_state_tup):
                ...
                with tf.variable_scope('decoder_rnn'):
                    weighting_context = weight_text * context + weight_style * sentence_style
                    weight_per = tf.reduce_mean(tf.abs(weight_style * sentence_style) / (tf.abs(weight_text * context) + tf.abs(weight_style * sentence_style)))
                ...
                return tf.add(this_time, 1), context, new_output_ta, new_alpha_ta, new_weight_ta, new_weight_per_ta, new_state_tup

            # run loop
            _, _, seq2seq_output_ta, alpha_ta, weight_ta, weight_per_ta, *_ = tf.while_loop(cond, body, [init_time, init_context, init_output_ta,
                                                                               init_alpha_ta, init_weight_ta, init_weight_per_ta, init_state_tup],
            ...
            with tf.variable_scope('reshape_decode'):
                ...
                alpha_output_style = sentence_style_att
                alpha_output_style = tf.expand_dims(alpha_output_style, -1)  # batch major
                alpha_output_style = tf.expand_dims(alpha_output_style, -1)  # batch major
                alpha_output_style = tf.transpose(alpha_output_style, perm=(0, 2, 1, 3))
    ...
    def get_scalar_summary(self, suffix):
        with tf.variable_scope('summary'):
            sums = [tf.summary.scalar('{}/loss'.format(suffix), self.loss),
                    ...
                    tf.summary.histogram('{}/alpha_style_0'.format(suffix), self.alpha_output_style[0]),
                    tf.summary.histogram('{}/alpha_style_2'.format(suffix), self.alpha_output_style[1]),
