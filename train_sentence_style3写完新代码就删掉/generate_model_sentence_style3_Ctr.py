...
class Tacotron(Model):
    def __init__(self, inp, inp_mask, decode_time_steps, ctr_flag, ctr_attention, hyper_params=None, name='Tacotron'):
        ...
        with tf.variable_scope(name):
            ...
            def body(this_time, old_output_ta, old_alpha_ta, old_weight_ta, old_weight_per_ta,
                ...
                with tf.variable_scope('decoder_rnn'):
                    ...
                    weight_per = tf.reduce_mean(tf.abs(weight_style * sentence_style) / (
                            tf.abs(weight_text * context) + tf.abs(weight_style * sentence_style)))
                ...
                return tf.add(this_time, 1), new_output_ta, new_alpha_ta, new_weight_ta,\
                       new_weight_per_ta, new_state_tup, context, new_output


            # run loop
            _, seq2seq_output_ta, alpha_ta, weight_ta, weight_per_ta, *_ = tf.while_loop(cond, body, [init_time,
                                                                                                                      init_output_ta,
                                                                                                                      init_alpha_ta,
                                                                                                                      init_weight_ta,
                                                                                                                      init_weight_per_ta,
