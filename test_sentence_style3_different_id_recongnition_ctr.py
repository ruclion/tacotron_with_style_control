...
from best_tacotron.generate_model_sentence_style3_Ctr import Tacotron
...
generate_path = os.path.join('logs', data_name) + '/generate_select_different_id_mult6/'
...
def main():
    ...
    model = Tacotron(inp, inp_mask, inp_att, decode_time_steps, hyper_params=hp)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        with open('selected_data_recongnition.pkl', 'rb') as f:
        ...
        for id_no in range(len(selected_data['batch_id'])):
            ...
            att = selected_data['batch_att'][id_no] * 6
            for i in range(len(selected_data['batch_id'])):
                ...
                for j in range(1):
                    ...
                    siowav.write(os.path.join(wav_folder, "audio%d_style_%d.wav" % (selected_data['batch_id'][i], now_id_no)), hp.sample_rate,
                                 pred_audio)

            pred_out = sess.run(model.post_output,
                                feed_dict={inp: txt_inp, inp_mask: txt_mask, inp_att: [att for _ in range(len(txt))],
                                           decode_time_steps: 60,
                                           })
            ...
            for j in range(len(txt)):
                ...
                if not os.path.exists(wav_folder):
                    os.makedirs(wav_folder)
                siowav.write(os.path.join(wav_folder, "test_audio%d_style_%d.wav" % (j, now_id_no)), hp.sample_rate,
                             pred_audio)
        '''
        #ctr, no style
        print('no style')
