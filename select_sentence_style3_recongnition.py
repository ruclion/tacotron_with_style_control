...
from best_tacotron.train_model_sentence_style3_recongnition import Tacotron
...
data_name = 'sr16_aB_sentence_style3_recongnition'
...
def get_next_batch(sess, next_item):
    ...
    return t['txt'], t['txt_len'], t['frames'], t['log_mel'], t['log_stftm'], t['key'], t['char_txt']
def post_next_batch(batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth, stats_meta):
    init_time_stamp = batch_mel_gtruth.shape[1]
    fix_time_stamp = init_time_stamp // hp.reduction_rate * hp.reduction_rate
    batch_frame_mask = np.minimum(batch_frame_mask, fix_time_stamp)
    ...
    #       )
    return batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth
...
def main():
    with tf.variable_scope('data'):
        ...
        frame_mask = tf.placeholder(name='frame_mask', shape=(None,), dtype=tf.int32)
    ...
    #
    train_model = Tacotron(inp=inp, inp_mask=inp_mask, frame_mask=frame_mask, seq2seq_gtruth=seq2seq_gtruth,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        select_lst['batch_id'] = []
        select_lst['batch_att'] = []
        while True:
            try:
                batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth, batch_key, batch_char_txt = get_next_batch(sess, train_next_item)
                # print('bug', batch_inp[0], 'len', batch_inp_mask[0], 'actual', batch_inp[0].shape)
                batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth = post_next_batch(
                    batch_inp, batch_inp_mask, batch_frame_mask, batch_mel_gtruth, batch_spec_gtruth, train_meta)
                for i, var in enumerate(batch_key):
                    ...
                    for j in select_key:
                        ...
                        if var.find(j) != -1:
                            ...
                            this_att = sess.run(
                                train_model.att,
                                feed_dict={inp: batch_inp, inp_mask: batch_inp_mask, frame_mask: batch_frame_mask,
                                           seq2seq_gtruth: batch_mel_gtruth,
                                           post_gtruth: batch_spec_gtruth})
                            select_lst['batch_att'].append(this_att[0])

            except Exception as e:
                print('stopped')
                break

        with open('selected_data_recongnition.pkl', 'wb') as f:
            pkl.dump(select_lst, f)
        for i in range(len(select_key)):
            print(select_lst['batch_key'][i], select_lst['batch_id'][i], select_lst['batch_char_txt'][i], select_lst['batch_att'][i])
